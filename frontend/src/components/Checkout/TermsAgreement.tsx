...
// A scrollable terms box + required checkbox gate, shown on the checkout
// page before "確認送出訂單" - the customer must actively tick the box
// (canSubmit in CheckoutPage.tsx depends on it) before an order can be
// placed, since these terms (no-returns on custom orders, warranty scope)
// only carry legal weight if genuinely agreed to, not just displayed.
export default function TermsAgreement({ agreed, onChange }: TermsAgreementProps) {
  return (
    <section className="mb-6 rounded-sm border border-line bg-panel p-4">
      <h2 className="mb-2.5 font-mono text-[11px] uppercase tracking-wide text-teal">服務條款</h2>

      <div className="max-h-64 overflow-y-auto rounded-sm border border-line bg-white p-3.5 text-[12.5px] leading-relaxed text-ink-dim">
        {TERMS.map((section) => (
          <div key={section.title} className="mb-4 last:mb-0">
            <div className="mb-1.5 font-semibold text-ink">{section.title}</div>
